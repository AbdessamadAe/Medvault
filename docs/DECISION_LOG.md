...
| Case tagging by body area | Free-text tags; user-picked specific organs; fixed list of ~15 broad body systems | Fixed list of body systems (Postgres enum array on `cases`), multi-select toggle chips | User explicitly wanted zero required medical/anatomy knowledge and minimal friction — a fixed, coarse, tappable list beats free text (drifting spellings, no filterability) or per-organ picking (requires anatomy knowledge) | Yes — user confirmed the proposed list |
